 ...
 setInterval(async function () {
    ...
    for (const server of servers) {
        await axios({
            ...
            client.sms.enviar(server.developer.telephone, message,options).then((response)=>{
...
setInterval(function(){
    for (const notification of notifications){
        if(notifications.status==="pending"){
            client.sms.buscar(notification.id).then(function(response){
                if (response.dados.resposta ==="1"){
                    ...
                    client.sms.enviar(boss.telephone, message, options)
